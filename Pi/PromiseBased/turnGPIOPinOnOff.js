...
const turnPins = ({ LOW_HIGH }) => {
  ...
  debug(
    `GPIO: ${immerse_heaters_pins.map(({ pinNumber }) => `${pinNumber}`)} ==> ${
      LOW_HIGH === "HIGH" ? "HIGH:on" : "LOW:off"
    }`,
    "blue"
  );
};
...
async function turnGPIOPinOnOff(previousPromise) {
  ...
  try {
    ...
    if (shouldTurnOn === undefined)
      throw new Error("{r1:{shouldTurnOn:true|false}} not defined");
    ...
    if (shouldTurnOn === true) {
      ...
      debug(
        `GPIO: ${immerse_heaters_pins.map(
          ({ pinNumber }) => `${pinNumber}`
        )} same state::${previouslySet}`,
        "blue"
      );
