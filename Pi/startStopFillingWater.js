...
const {
  showActiveNonActivePin,
} = require("../Pi/PromiseBased/turnGPIOPinOnOff");
...
const startStopFillingWater = ({ stop } = { stop: false }) => {
  ...
  debug(
    `Pin ${pinNumber} is initial ${rpio.read(pinNumber) ? "high" : "low"}`,
    "blue"
  );
  ...
  const pollcb = (pin) => {
    ...
    setTimeout(() => {
      if (water_curcuit_closed) {
        ...
        rpio.poll(pin, null);
        debug(
          `${pinNumber} job completed ! ${
            rpio.read(pinNumber) ? "high" : "low"
          }`,
          "blue"
        );
